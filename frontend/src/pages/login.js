        ...
        http(url).then((mensaje) => {
            if (!mensaje.error) {
                ...
                mensaje.grupos.map((grupo) => {
                    if (
                        grupo.cn.toUpperCase().includes("BETICA ACCESO-APPPQ")
                    ) {
                        contador = contador + 1;
                        tieneGrupoPQ = true;
                ...
                if (!tieneGrupoPQ) {
                    setMensajeError(
                        "No tiene permisos de acceso a la aplicación."
                    );
                    setErrorVisible(true);
                } else {
                    if (contador > 1) {
                        setMensajeError(
                            "El usuario tiene mas de un centro asignado."
                        );
                    } else {
                        //Login OK
                        dispatch({ type: "SET_LOGIN_OK", payload: true });
                        var user = {
                            loginUser: mensaje.saMAccountname,
                            nombre: mensaje.displayName,
                            email: mensaje.userPrincipalName,
                            codCentro: nombreGrupo.substring(21, 23),
                        };
                        alert(user.codCentro);
